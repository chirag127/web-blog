...
async function syncCentralRepo() {
  ...
  if (!fs.existsSync(SECRETS_DIR)) {
    ...
    const gitUrl = await question(
      "Enter the Git URL of your private secrets repository: ",
    );
...
async function main() {
  ...
  const isSyncOnly = args.includes("sync");
  ...
  for (const key of Object.keys(neededKeys)) {
    ...
    if (typeof rawValue === "string" && rawValue.startsWith("shared.")) {
      ...
      if (sharedValue === undefined) {
        ...
        const val = await question(
          `Enter value for shared secret ${sharedKey}: `,
        );
  ...
  if (modified) {
    ...
    if (fs.existsSync(path.join(SECRETS_DIR, ".git"))) {
      ...
      try {
        ...
        console.log(
          "\x1b[32m%s\x1b[0m",
          "Pushed central secrets successfully.",
        );
      } catch (err) {
        console.error(
          "Failed to commit/push secrets.json to remote repo:",
          err.message,
        );
      }
    }
  ...
  if (isGithubSync) {
    console.log(
      "\n\x1b[35m%s\x1b[0m",
      `Syncing secrets to GitHub repository: ${repo.fullName || repo.name}`,
    );
    ...
    try {
      ...
      console.log(
        "\x1b[32m%s\x1b[0m",
        "Successfully synced all secrets to GitHub Actions!",
      );
    } catch (err) {
      console.error(
